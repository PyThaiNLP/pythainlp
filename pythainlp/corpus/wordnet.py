...
def lemma_from_key(key):
    """
        ...
        This is similar to :func:`lemma` but it needs to supply the key of lemma instead of the name.
...
def path_similarity(synsets1, synsets2):
    """
        ...
        The shortest path distance is calculated by the connection through the is-a (hypernym/hyponym) taxonomy.
        The score is in the ranage 0 to 1. Path similarity of 1 indicates identicality.

        :param `Synset` synsets1: first synset supplied to measures the path similarity
        :param `Synset` synsets2: second synset supplied to measures the path similarity
...
def lch_similarity(synsets1, synsets2):
    """
        This function returns Leacock Chodorow similarity (LCH) between two synsets,
        based on the shortest path distance and the maximum depth of the taxonomy.
        The equation to calculate LCH similarity is shown below:
    
        .. math::

            lch\_similarity = {-log(shortest\_path\_distance(synsets1, synsets2) \over 2 * taxonomy\_depth}

        :param `Synset` synsets1: first synset supplied to measures the LCH similarity
        :param `Synset` synsets2: second synset supplied to measures the LCH similarity
...
def wup_similarity(synsets1, synsets2):
    """
        ...
        based on the depth of the two senses in the taxonomy and their Least Common Subsumer (most specific ancestor node).
    
        :param `Synset` synsets1: first synset supplied to measures the WUP similarity
        :param `Synset` synsets2: second synset supplied to measures the WUP similarity
...
def morphy(form, pos: str = None):
    """
        This function finds a possible base form for the given form, with the given
        part of speech.
